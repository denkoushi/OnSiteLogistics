...
- USB ベースで運用している要領書／工具マスタの配布について、今回構築した HTTP + 再送キュー基盤を活用したサーバー連携案を検討。対象データの API 化と段階的な移行ロードマップを作成する。
- 工場 LAN への本番移行に備え、アクセスポイント構成（2.4 GHz の電波品質）、サーバーの固定 IP／DNS 設定、UFW の許可範囲、TLS 化（必要に応じてリバースプロキシ）を整備。移行前にハンディリーダの再送キュー挙動・Socket.IO 更新の動作確認を現地で実施する。
