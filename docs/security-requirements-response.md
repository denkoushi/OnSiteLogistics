...
| 要求項目 | 内容 | 対応状況 | 備考 |
| --- | --- | --- | --- |
| 認証情報の保護 | API トークンを安全に保管し、最低限の権限とする | ✅ config は `root:root 640`。Pi5 側で発行・ローテーション（RaspberryPiServer RUNBOOK 4章）。`install_client_config.sh` がバックアップを暗号化せず残すため、不要になったバックアップは手動削除する。 |
| 通信経路保護 | 送信データを盗聴・改ざんから守る | ☐ 工場内 LAN + WPA2 のみ。将来の HTTPS/TLS 化を Pi5 側と調整中。 |
| 端末紛失対策 | 端末紛失・盗難時の情報漏えいを抑える | ☐ ローカルキューは平文 SQLite。紛失時は Pi5 トークンを失効し、端末を再イメージする手順を RUNBOOK（3章）に追記予定。物理ロック・管理台帳で補完。 |
